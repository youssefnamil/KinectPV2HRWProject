void initPhysics() {
  ...
  physics1.addBehavior(new GravityBehavior2D(new Vec2D(0,0.1)));
  ...
  for(int i=0; i<STRING_RES; i++) {
    ...
    physics1.addBehavior(new AttractionBehavior2D(p,delta*1.5,-20));
  ...
  for(int i=0; i<BALL_RES; i++) {
    ...
    physics1.addSpring(new VerletSpring2D(c,p,BALL_RADIUS,0.01));
